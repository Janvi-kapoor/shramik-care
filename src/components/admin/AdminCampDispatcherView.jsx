...
export const AdminCampDispatcherView = () => {
  ...
  const fetchDoctors = async () => {
    try {
      const json = await adminApi('/doctors');
      setDoctors(json);
    } catch (err) {
  ...
  const fetchCamps = async () => {
    setLoading(true);
    try {
      const json = await adminApi('/camps');
      setCamps(json);
    } catch (err) {
      console.error(err);
  ...
  const handleDispatch = async (e) => {
    ...
    const resources = Array.from(e.target.querySelectorAll('input[name="resources"]:checked')).map(input => input.value);

    if (!date) {
      showToast('Please select a date for the camp', 'error');
      return;
    }
    if (capacity <= 0) {
    ...
    try {
      await adminApi('/camps', {
        method: 'POST',
        body: JSON.stringify({ name, district, location, date, purpose, capacity, assigned_doctor_ids, resources })
      });

      showToast(`Health camp deployed successfully.`, 'success');
      e.target.reset();
      fetchCamps();
    } catch (err) {
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
      <div className="flex items-center justify-between mb-6 border-b border-slate-100 pb-4">
        <div className="flex items-center space-x-2">
          <Bus className="w-5 h-5 text-[#5a52d9]" />
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="p-5 rounded-xl border border-teal-100 bg-teal-50 shadow-sm">
            ...
            <form onSubmit={handleDispatch} className="space-y-4">
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Camp Name</label>
                <input name="name" type="text" required placeholder="e.g. Perumbavoor Fever Screening Camp" className="w-full p-2.5 rounded-lg border border-slate-300 text-sm focus:ring-1 focus:ring-teal-500 bg-white shadow-sm" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Required Resources</label>
                <div className="grid grid-cols-2 gap-2 text-xs font-semibold text-slate-700">
                  {['Doctor', 'Nurse', 'Basic medicines', 'Screening equipment', 'First-aid supplies'].map(resource => <label key={resource} className="flex gap-2 items-center"><input type="checkbox" name="resources" value={resource} defaultChecked={resource === 'Doctor'} className="accent-teal-700" />{resource}</label>)}
                </div>
              </div>
              <div>
              ...
              <div key={camp.id} className="p-4 rounded-xl border border-slate-200 bg-white shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-sm font-bold text-slate-900">{camp.name || camp.purpose}</span>
                  </div>
                  ...
                  <div className="text-xs text-slate-500 mt-1 flex items-center gap-3">
                    ...
                    <span className="flex items-center gap-1"><Users className="w-3 h-3"/> {camp.enrolled}/{camp.capacity} Registered · {camp.available} Available</span>
                  </div>
                </div>
                <div>
