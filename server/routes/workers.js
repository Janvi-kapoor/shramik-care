    ...
    (err, prescriptions) => {
      ...
      const promises = prescriptions.map((p) => {
        return new Promise((resolve, reject) => {
          db.all(
            "SELECT * FROM prescription_medicines WHERE prescriptionId = ?",
            [p.id],
            (err, medicines) => {
              if (err) reject(err);
...
router.post("/:id/prescriptions", (req, res) => {
  ...
  db.run(
    `INSERT INTO prescriptions (id, workerId, doctorId, doctorName, date, diagnosis)
          VALUES (?, ?, ?, ?, ?, ?)`,
...
router.post("/enroll-camp", (req, res) => {
  ...
  db.get("SELECT * FROM camps WHERE id = ?", [campId], (campErr, camp) => {
    ...
    db.get(
      ...
      (countErr, row) => {
        ...
        if (row.count >= camp.capacity)
          return res.status(409).json({ error: "This camp is full" });
        createEnrollment();
      },
    );
  });
  ...
  function createEnrollment() {
    db.get(
      `SELECT id FROM camp_enrollments WHERE workerId = ? AND campId = ?`,
      [workerId, campId],
      (err, existing) => {
        if (err) return res.status(500).json({ error: err.message });
        if (existing) {
          return res
            .status(400)
            .json({ error: "Worker is already enrolled in this camp" });
        }

        db.run(
          `INSERT INTO camp_enrollments (id, campId, workerId, token, status) VALUES (?, ?, ?, ?, 'Registered')`,
          [enrollId, campId, workerId, token],
          function (err) {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ success: true, token, enrollmentId: enrollId });
          },
        );
      },
    );
...
router.get("/:id/notifications", (req, res) => {
  db.all(
    "SELECT * FROM worker_notifications WHERE workerId = ? ORDER BY created_at DESC",
    [req.params.id],
    (err, rows) => {
      if (err) return res.status(500).json({ error: err.message });
...
router.put("/:id/notifications/:notifId/read", (req, res) => {
  db.run(
    "UPDATE worker_notifications SET is_read = 1 WHERE id = ? AND workerId = ?",
